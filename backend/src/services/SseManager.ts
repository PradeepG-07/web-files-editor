...
export default class SseManager {
    private static connections: Map<string, Response> = new Map<
        ...
        Response
    >();
    static async registerSubscriptionInRedis(
        clientId: string,
        subscribeToPath: string
    ) {
        const response = await RedisManager.addEntry(
            RedisPrefixes.REGISTERED_SUBSCRIPTIONS,
            clientId,
            subscribeToPath
        );
        return response;
    ...
    static async createSubscription(subscribeToPath: string) {
        ...
        const registeredInRedis = await this.registerSubscriptionInRedis(
            clientId,
            subscribeToPath
        );
    ...
    static async addConnection(clientId: string, clientConnection: Response) {
        ...
        const subscribedPath = (await RedisManager.getEntry(
            RedisPrefixes.REGISTERED_SUBSCRIPTIONS,
            clientId
        )) as string;
        ...
        const subscribedClients = await RedisManager.getEntry(
            RedisPrefixes.SERVER_SUBSCRIPTIONS,
            subscribedPath
        );
        ...
        await RedisManager.addEntry(
            RedisPrefixes.SERVER_SUBSCRIPTIONS,
            subscribedPath,
            newSubscribedClients
        );
    ...
    static async pushUpdatesToConnections(
        message: string,
        subscriptionPath: string
    ) {
        const subscribedClientIds = (await RedisManager.getEntry(
            RedisPrefixes.SERVER_SUBSCRIPTIONS,
            subscriptionPath
        )) as Array<string>;
        const { event, data } =
            RedisPubSubService.formatSubscribedMessage(message);
    ...
    static async closeConnection(clientId: string) {
        ...
        await RedisPubSubService.decrementSubscriptionCountForChannel(
            subscribedPath
        );
    }
}
