...
export default class RedisManager {
    ...
    static async addEntry<k extends RedisPrefix>(
        prefix: RedisPrefix,
        key: string,
        data: RedisPrefixDataMap[k]
    ): Promise<boolean> {
        ...
        return new Promise<boolean>(async (resolve) => {
            ...
            if (prefix == RedisPrefixes.REGISTERED_SUBSCRIPTIONS) {
                const promise = client.set(
                    `${prefix}-${this.serverId}:${key}`,
                    data as string
                );
