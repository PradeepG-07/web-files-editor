...
export const registerToSSE = asyncHandler(async (req: Request, res: Response) => {
    ...
    if(streamUrl == null) throw new ApiError(500, "Unable to subscribe","Unable to subscribe");
    res.json(new ApiRespone(200,{streamUrl},"Subscribed to SSE"));
});

export const initialiseSSE = asyncHandler(async (req: Request, res: Response) => {
    const clientId = req.params.clientId;
    const connected = await SseManager.addConnection(clientId,res);
    if(!connected){
        throw new ApiError(400,"Connection not found","Could not able to initialise SSE");
    }
    res.writeHead(200,{
