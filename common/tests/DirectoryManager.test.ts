...
describe('DirectoryManager', () => {
    ...
    const DIRECTORY = "/home/pradeep.g/Projects/web-files-editor/common/test-space/";
    describe('Is not ignored directory', () => { 
        it('Should return true',async()=>{
            const validPath = "/home/pradeep.g/temp";
            const result = await DirectoryManager.checkIsNotIgnoredDirectory(validPath);
        ...
        it("Should throw Invalid Directory",async()=>{
            ...
            try {
                ...
                expect(e).toBe(Messages.INVALID+Messages.SPACE+Messages.DIRECTORY+Messages.SPACE+Messages.PATH);
            }
        })
     })
    describe('Is Valid Directory', () => { 
        it('Should return true',async()=>{
            const validPath = "/home/pradeep.g/temp";
            const result = await DirectoryManager.checkIsValidDirectory(validPath);
            expect(result).toBe(true);
        });
        it("Should throw Invalid Directory",async()=>{
            const invalidPath = "/data/temp.txt";
    ...
    describe("Directory Exists",()=>{
        it("Should return true",async ()=>{
            const result = await DirectoryManager.checkIsDirectoryExist(DIRECTORY);
            expect(result).toBe(true);
        });
        it("Should throw Directory does not exit",async ()=>{
            ...
            try {
                ...
                expect(errorMsg).toBe(Messages.DIRECTORY+Messages.SPACE+Messages.DOES_NOT_EXIST);
            }
        });
    })
    describe('Create Directory', () => {
        it('Should create a Directory', async() => {
            ...
            jest.spyOn(DirectoryManager,"checkIsDirectoryExist").mockRejectedValueOnce("");
        ...
        it('Should fail to create file as file already exists', async() => {
            try {
                jest.spyOn(DirectoryManager,"checkIsDirectoryExist").mockResolvedValueOnce(true);
                await DirectoryManager.createDirectory(DIRECTORY);
            } catch (errorMsg) {
                expect(errorMsg).toBe(Messages.DIRECTORY+Messages.SPACE+Messages.ALREADY_EXIST);
            }
        });
    });
    ...
    describe('Delete Directory', () => {
        ...
        it('Should fail to delete directory as it does not exist', async() => {
            try {
                jest.spyOn(DirectoryManager,"checkIsDirectoryExist").mockRejectedValue(Messages.DIRECTORY+Messages.SPACE+Messages.DOES_NOT_EXIST);
