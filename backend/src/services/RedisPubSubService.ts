...
export default class RedisPubSubService {
    ...
    static async subscribeToChannel(channel: string) {
        if (!this.subscribedChannels.has(channel)) {
            const client = await RedisClient.getClient(
                this.connString,
                RedisClientMode.SUBSCRIBE
            );
            await client.subscribe(channel, this.updatesListener);
        }
        const numberOfConnectionsSubscribed =
            this.subscribedChannels.get(channel) ?? 0;
    ...
    static async decrementSubscriptionCountForChannel(channel: string) {
        const numberOfConnectionsSubscribed =
            (this.subscribedChannels.get(channel) ?? 0) - 1;
        console.log(
            `Channel: ${channel} connections: ${numberOfConnectionsSubscribed}`
        );
